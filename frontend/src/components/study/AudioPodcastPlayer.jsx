...
import { Play, Pause, Download, Volume2, Loader } from 'lucide-react';
...
export default function AudioPodcastPlayer({ summaryText }) {
  ...
  const generateAudio = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/study/audio', {
        method: 'POST',
        headers: {
    ...
    <div className="bg-gradient-to-br from-indigo-900 to-purple-900 rounded-xl p-6 text-white">
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-300">
            {isLoading ? 'Generating audio...' : 'Powered by DeepGram Nova'}
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-8">
          ...
          <span className="ml-3">Generating high-quality audio...</span>
        </div>
      ) : (
        <>
          {/* Progress Bar */}
